...
import offscreenApiProxy from '@deushq/kit-bg/src/offscreens/instance/offscreenApiProxy';
import { getExtensionIndexHtml } from '@deushq/kit/src/utils/extUtils.getHtml';
...
function initBackground() {
  const backgroundApiProxy: typeof import('@deushq/kit/src/background/instance/backgroundApiProxy').default =
    // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
    require('@deushq/kit/src/background/instance/backgroundApiProxy').default;
...
console.log(
  `[Deus RN] Extension background page ready: 666  ${new Date().toLocaleTimeString()}`,
);
initBackground();
