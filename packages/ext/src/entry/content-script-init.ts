...
import '@deushq/shared/src/polyfills/polyfillsExtContentScript';
...
import type { IDeusWalletInfo } from '@deushq/kit-bg/src/providers/ProviderApiPrivate';
...
if (process.env.NODE_ENV !== 'production') {
  ...
  console.log('[Deus RN]: Content script works! ', window.location.href);
...
if (process.env.NODE_ENV !== 'production') {
  try {
    if (consts.WALLET_INFO_LOACAL_KEY) {
      const walletInfo: IDeusWalletInfo = JSON.parse(
