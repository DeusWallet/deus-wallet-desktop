...
export function validateTypedSignMessageDataV3V4(
  ...
  if (validation.errors.length > 0) {
    throw new DeusError('Message Data must conform to EIP-712 schema.');
  }

  if (!currentChainId) {
    throw new DeusError('Current chainId cannot be null or undefined.');
  }
  ...
  if (chainId) {
    ...
    if (activeChainIdBN.isNaN()) {
      throw new DeusError(
        `Cannot sign messages for chainId "${chainIdBN.toFixed()}", because Deus is switching networks.`,
      );
    }
