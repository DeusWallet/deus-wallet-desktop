...
import {
  FileLogger,
  LogLevel,
} from '@deushq/shared/src/modules3rdParty/react-native-file-logger';
import RNFS from '@deushq/shared/src/modules3rdParty/react-native-fs';
import { zip } from '@deushq/shared/src/modules3rdParty/react-native-zip-archive';
...
const NATIVE_LOG_DIR_PATH = `${RNFS?.CachesDirectoryPath || 'Deus'}/logs`;
const NATIVE_LOG_ZIP_DIR_PATH = `${
  RNFS?.CachesDirectoryPath || 'Deus'
}/log_zip`;
...
> = {
  ...
  [LoggerNames.deusLite]: Cache.createLogger(LoggerNames.deusLite),
...
const DEBUG_LOGGER_STORAGE_KEY = '$$DEUS_DEBUG_LOGGER';
