...
ctx.fetch = require('@deushq/shared/src/request/normalize/normalizeCrossFetch').normalizeCrossFetch({
  fetch: ctx.fetch
});
...
export function normalizeCrossFetch({
  ...
  // @ts-ignore
  if (fetch && fetch.isNormalizedByDeus) {
    return fetch;
  }
  ...
  newFetch.isNormalizedByDeus = true;

  // @ts-ignore
  if (global.fetch && !global.fetch.isNormalizedByDeus) {
