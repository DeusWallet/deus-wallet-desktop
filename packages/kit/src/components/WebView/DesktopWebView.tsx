...
import { consts } from '@deusfe/cross-inpage-provider-core';
import { JsBridgeDesktopHost } from '@deusfe/deus-cross-webview';
...
import { checkDeusCardGoogleOauthUrl } from '../../utils/uriUtils';
...
function usePreloadJsUrl() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      Please send messages of channel SET_DEUS_DESKTOP_GLOBALS at app start
      `);
      }
    }, 3000);
    ...
    useEffect(() => {
      ...
      try {
        const checkGoogleOauth = (checkUrl: string) => {
          try {
            if (checkDeusCardGoogleOauthUrl({ url: checkUrl })) {
