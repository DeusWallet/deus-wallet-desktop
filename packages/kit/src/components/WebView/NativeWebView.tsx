...
import { JsBridgeNativeHost } from '@deusfe/deus-cross-webview';
...
import type { InpageProviderWebViewProps } from '@deusfe/cross-inpage-provider-types';
...
const NativeWebView = forwardRef(
  (
    ...
    const webViewOnLoadStart = useCallback((syntheticEvent) => {
      ...
      try {
        if (checkDeusCardGoogleOauthUrl({ url })) {
          openUrlExternal(url);
