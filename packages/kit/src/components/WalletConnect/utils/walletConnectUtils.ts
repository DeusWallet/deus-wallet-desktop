...
import { DEUS_APP_DEEP_LINK } from '../walletConnectConsts';
...
if (platformEnv.isNative || platformEnv.isDesktop) {
  connectionRedirectUrl = DEUS_APP_DEEP_LINK;
...
function buildDeusWalletConnectDeepLinkUrl({ uri }: { uri: string }) {
  return `${DEUS_APP_DEEP_LINK}/wc?uri=${encodeURIComponent(uri || '')}`;
}
...
function convertToSessionNamespacesV2({
      ...
      if (key !== 'eip155' && !key.startsWith('eip155:')) {
        ...
        if (isRequired) {
          ...
          const error = new Error(
            'Deus WalletConnect ERROR: EVM chain supported only',
          );
          onError?.(error);
          throw error;
        }
  ...
  if (hasNonEvmChain && !hasEvmChain) {
    // throw new Error(
    //   'Deus WalletConnectV2 Non-EVM chain not supported yet.',
    // );
  }
...
export default {
  ...
  buildDeusWalletConnectDeepLinkUrl,
  isWalletConnectV2,
