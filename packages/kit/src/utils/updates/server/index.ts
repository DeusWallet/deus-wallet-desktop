...
export async function getReleaseInfo(): Promise<PackagesInfo | null> {
  ...
  return axios
    .get<AppReleases>(`https://data.deus.so/config.json?nocache=${key}`)
...
export async function getPreReleaseInfo(): Promise<PackagesInfo | null> {
  const key = Math.random().toString();
  return axios
    .get<AppReleases>(`https://data.deus.so/pre-config.json?nocache=${key}`)
    .then((releasesVersionResponse) => {
      const releasesVersion = releasesVersionResponse.data;
      return handleReleaseInfo(releasesVersion);
    });
...
export async function getChangeLog(
  ...
  let changelogUrl = `https://data.deus.so/config.json`;
  if (isPreRelease) {
    changelogUrl = `https://data.deus.so/pre-config.json`;
  }
