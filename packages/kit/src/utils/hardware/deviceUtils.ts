...
import { HardwareErrorCode, createDeferred } from '@deusfe/hd-shared';
...
import { toPlainErrorObject } from '@deushq/shared/src/utils/errorUtils';
...
import { getDeviceFirmwareVersion } from './DeusHardware';
...
import type { Deferred } from '@deusfe/hd-shared';
...
class DeviceUtils {
  ...
  showErrorToast(error: any, defKey?: LocaleIds): boolean {
    ...
    try {
      ...
      if (code === Error.CustomDeusHardwareError.NeedDeusBridge) {
        this.delayShowHardwarePopup({
          uiRequest: CUSTOM_UI_RESPONSE.CUSTOM_NEED_DEUS_BRIDGE,
        });
        return true;
      }

      if (className === DeusErrorClassNames.DeusHardwareError) {
        const { data } = error || {};
        ...
        // Ignore key
        const ignoreKeys = ['msg__engine__internal_error', 'deus_error'];
  ...
  convertDeviceError(payload: any): DeusHardwareError {
    // handle ext error
    const {
      code,
      error,
    ...
    switch (code) {
      ...
      case HardwareErrorCode.BridgeNotInstalled:
        return new Error.NeedDeusBridge(payload);
      case Error.CustomDeusHardwareError.NeedDeusBridge:
        return new Error.NeedDeusBridge(payload);
