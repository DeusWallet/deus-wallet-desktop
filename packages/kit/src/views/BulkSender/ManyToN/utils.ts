...
const randomBetween = ({
  ...
  if (minBN.isGreaterThan(maxBN)) {
    throw new DeusError('Min must be less than or equal to max');
  }
...
export const getTransferAmount = async ({
  ...
  if (amountType === AmountTypeEnum.Custom) {
    if (isNil(senderItem.amount)) {
      throw new DeusError('Can not get sender amount value');
    }
...
export const verifyBulkTransferBeforeConfirm = async ({
  ...
  for (let i = 0; i < sender.length; i += 1) {
    ...
    if (isNil(senderNativeTokenBalance) || isNil(senderTokenBalance)) {
      throw new DeusError('Can not get sender balance value.');
    }
