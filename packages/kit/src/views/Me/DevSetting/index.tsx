...
import type { ISettingsDevModeInfo } from '@deushq/kit/src/store/reducers/settings';
import {
  setDevMode,
  setEnablePerfCheck,
...
import timelinePerfTrace from '@deushq/shared/src/perf/timelinePerfTrace';
...
interface IDeusPerfCheckPayload {
  testID?: string;
...
function usePerfCheck({ enablePerfCheck }: { enablePerfCheck?: boolean }) {
  useEffect(() => {
    if (enablePerfCheck && platformEnv.isRuntimeBrowser) {
      ...
      window.addEventListener('DeusEventPerfCheck', handler);
      return () => {
        window.removeEventListener('DeusEventPerfCheck', handler);
      };
    }
  }, [enablePerfCheck]);
}
  ...
  return (
    <Box w="full" mb="6">
      ...
      >
        ...
        <Container.Item
          title={intl.formatMessage({ id: 'action__test_deus_service' })}
          subDescribe={`范围: \n[token、价格、余额、推送、历史记录] \n ${fiatEndpoint}\n ${getSocketEndpoint()} \n ${getCovalentApiEndpoint()}`}
          titleColor="text-critical"
        >
          <Switch
          ...
          <HStack space={4}>
            <Button
              ...
              onPress={() => {
                ...
                copyToClipboard(
                  JSON.stringify({
                    $$deusPerfTrace: global?.$$deusPerfTrace,
                    perfCheckResult,
                ...
                console.log('$$deusPerfTrace', global?.$$deusPerfTrace);
