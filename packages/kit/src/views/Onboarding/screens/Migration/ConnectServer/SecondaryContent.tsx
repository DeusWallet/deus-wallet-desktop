...
import type {
  DeviceInfo,
  MigrateData,
} from '@deushq/engine/src/types/migrate';
import { MigrateErrorCode } from '@deushq/engine/src/types/migrate';
import { httpServerEnable } from '@deushq/kit-bg/src/services/ServiceHTTP';
import { MigrateNotificationNames } from '@deushq/kit-bg/src/services/ServiceMigrate';
import type { MigrateNotificationData } from '@deushq/kit-bg/src/services/ServiceMigrate';
import qrcodeLogo from '@deushq/kit/assets/qrcode_logo.png';
import { RootRoutes } from '@deushq/kit/src/routes/routesEnum';
import {
  AppUIEventBusNames,
  appUIEventBus,
} from '@deushq/shared/src/eventBus/appUIEventBus';
  ...
  return (
    <>
      {qrcode.length > 0 ? (
        ...
        >
          <QRCode
            value={`${DeusMigrateQRCodePrefix}${qrcode}`}
            size={170}
            logo={qrcodeLogo}
            logoSize={36}
            logoMargin={5}
      ...
      <Input
        ...
        rightCustomElement={
          <IconButton
            ...
            onPress={() => {
              gotoScanQrcode((data) => {
                if (data.startsWith(DeusMigrateQRCodePrefix)) {
                  if (setContext) {
                    setContext((ctx) => ({
                      ...ctx,
                      inputValue: data.replace(DeusMigrateQRCodePrefix, ''),
                    }));
                  }
                }
              });
