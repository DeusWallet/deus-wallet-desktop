...
import { Box, Center, ToastManager } from '@deushq/components';
...
function BehindTheSceneCreatingWallet({
  ...
  const startCreatingHardwareWallet = useCallback(async () => {
    try {
      ...
      const features: IDeusDeviceFeatures | undefined =
      ...
      */
      ...
      if (className === DeusErrorClassNames.DeusAlreadyExistWalletError) {
        ...
        onPressOnboardingFinished?.();
      } else if (className === DeusErrorClassNames.DeusHardwareError) {
