...
import DeviceMobile from '@deushq/kit/assets/onboarding/device_classic_touch.png';
import DeviceAll from '@deushq/kit/assets/onboarding/device_mini_classic_touch.png';
// eslint-disable-next-line @typescript-eslint/no-unused-vars
import ImgMetaMask from '@deushq/kit/assets/onboarding/logo_metamask.png';
import DeusLite from '@deushq/kit/assets/deus-lite.png';
  ...
  return (
    ...
    >
      <Text typography="Heading" maxW={{ base: 160, sm: 'auto' }}>
        {intl.formatMessage({ id: 'action__connect' })} Deus{'\n'}
...
function ConnectDeusLiteButton() {
  const navigation = useAppNavigation();

  const context = useOnboardingContext();
  const forceVisibleUnfocused = context?.forceVisibleUnfocused;

  const showDeusLiteModal = useCallback(() => {
    forceVisibleUnfocused?.();
    navigation.navigate(RootRoutes.Modal, {
      screen: ModalRoutes.CreateWallet,
      params: {
  ...
  return (
    <Box mt={{ base: 6, sm: 4 }} mx={{ base: -2, sm: 0 }}>
      <Pressable
        ...
        _hover={{ bgColor: 'surface-hovered' }}
        _pressed={{ bgColor: 'surface-pressed' }}
        onPress={() => showDeusLiteModal()}
      >
        <Image source={DeusLite} size={8} />
        <Text
          flex={1}
          mx={3}
