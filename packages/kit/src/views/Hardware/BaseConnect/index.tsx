...
import Button from '@deushq/components/src/Button';
import iconNFCScanHint from '@deushq/kit/assets/hardware/ic_pair_hint_scan_lite.png';
...
const HardwareConnect: FC<HardwareConnectViewProps> = ({
  ...
  const [lottieConnectingIcon, setLottieConnectingIcon] = useState<any>(
    require('@deushq/kit/assets/animations/lottie_deus_lite_nfc_connect.json'),
  );
  ...
  useEffect(() => {
    ...
    if (operateType === 'transfer') {
      setLottieConnectingIcon(
        require('@deushq/kit/assets/animations/lottie_deus_lite_nfc_transfer.json'),
      );
      setTimeout(() => {
        setLottieConnectingIcon(
          require('@deushq/kit/assets/animations/lottie_deus_lite_nfc_transmitting.json'),
        );
      }, 1000);
      if (platformEnv.isNativeIOS) {
        setVisibleIosHint(false);
    ...
    if (operateType === 'complete' || operateType === 'done') {
      setLottieConnectingIcon(
        require('@deushq/kit/assets/animations/lottie_deus_lite_nfc_complete.json'),
      );
      if (platformEnv.isNativeIOS) {
        setVisibleIosHint(false);
      }
  ...
  return (
    <>
      <Modal
        ...
        header={title}
        onClose={() => {
          debugLogger.deusLite.debug('HardwareConnect: onClose');
        ...
        <Box w="100%" p={4} h="212px" position="absolute">
          ...
          >
            ...
            <Typography.DisplayMedium px={12} mt={5} textAlign="center">
              {intl.formatMessage({
                id: 'content__place_your_deus_lite_close_to_the_back_of_here',
              })}
