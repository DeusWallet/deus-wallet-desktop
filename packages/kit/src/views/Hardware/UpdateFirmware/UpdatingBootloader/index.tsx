...
import { Modal, ToastManager } from '@deushq/components';
...
import * as Errors from '@deushq/kit/src/utils/hardware/errors';
import {
  AppUIEventBusNames,
  appUIEventBus,
...
const UpdatingBootloader: FC = () => {
  ...
  const deviceType = useMemo(
    () => device?.deviceType as IDeusDeviceType | undefined,
    [device],
  );
  ...
  const handleErrors = (error: DeusHardwareError) => {
    ...
    switch (code) {
      ...
      case HardwareErrorCode.FirmwareUpdateAutoEnterBootFailure:
        ...
        break;
      default:
        if (className === 'DeusHardwareError') {
          setStateViewInfo({
            type: 'common_error',
            content: {
              title: intl.formatMessage({
  ...
  const executeUpdate = async () => {
    ...
    try {
      ...
      handleErrors(e as DeusHardwareError);
      return;
    }
    serviceHardware
