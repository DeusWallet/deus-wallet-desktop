...
import type { SelectItem } from '@deushq/components/src/Select';
...
import WebView from '@deushq/kit/src/components/WebView';
import { useActiveWalletAccount } from '@deushq/kit/src/hooks';
import type {
...
import {
  ...
  ModalRoutes,
  DeusLiteChangePinModalRoutes,
  DeusLiteResetModalRoutes,
  RootRoutes,
...
type NavigationProps = ModalScreenProps<CreateWalletRoutesParams> &
  ...
  ModalScreenProps<DeusLiteResetRoutesParams>;

const DeusLiteDetail: FC = () => {
  const intl = useIntl();
  ...
  useEffect(() => {
    setUrl(`https://lite.deus.so/?language=${locale}`);
  }, [locale]);
  ...
  const menuOptionsMemo = useMemo(() => {
    ...
    menuOptions.push({
      label: intl.formatMessage({
        id: 'action__restore_with_deus_lite',
      }),
      value: 'restore',
  ...
  const headerRightSelect = useCallback(
    () => (
      <Select
        ...
        title={intl.formatMessage({ id: 'app__hardware_name_deus_lite' })}
        onChange={(v) => {
  ...
  const startRestorePinVerifyModal = () => {
    navigation.navigate(RootRoutes.Modal, {
      ...
      params: {
        screen: CreateWalletModalRoutes.DeusLiteRestorePinCodeVerifyModal,
      },
    });
  };
  ...
  const startChangePinInputPinModal = () => {
    navigation.navigate(RootRoutes.Modal, {
      screen: ModalRoutes.DeusLiteChangePinInputPin,
      params: {
        screen: DeusLiteChangePinModalRoutes.DeusLiteChangePinInputPinModal,
      },
    });
  };
  ...
  const startResetModal = () => {
    navigation.navigate(RootRoutes.Modal, {
      screen: ModalRoutes.DeusLiteReset,
      params: {
        screen: DeusLiteResetModalRoutes.DeusLiteResetModal,
      },
    });
  };
    ...
    if (controlledWallets.length) {
      return (
        <Button
          ...
          type="primary"
        >
          {intl.formatMessage({ id: 'action__back_up_to_deus_lite' })}
        </Button>
      );
    }
    return (
      ...
      >
        {intl.formatMessage({ id: 'action__restore_with_deus_lite' })}
      </Button>
    );
    ...
    <>
      <Box flexDirection="column" flex={1}>
        ...
        <Dialog
          ...
          contentProps={{
            iconType: 'danger',
            title: intl.formatMessage({ id: 'action__reset_deus_lite' }),
            content: intl.formatMessage(
              {
                id: 'modal__reset_deus_lite_desc',
              },
              { 0: 'RESET' },
            ),
          }}
