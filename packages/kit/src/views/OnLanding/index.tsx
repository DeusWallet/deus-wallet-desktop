...
import { Button, Center, Icon, Typography } from '@deushq/components';
...
export default function OnLanding() {
  ...
  const onAndroidDownload = useCallback(() => {
    openUrlExternal(
      'https://play.google.com/store/apps/details?id=so.deus.app.wallet',
    );
  }, []);
  const oniOSDownload = useCallback(() => {
    openUrlExternal(
      'https://apps.apple.com/us/app/deus-open-source-wallet/id1609559473',
    );
  }, []);
  const onDesktopDownload = useCallback(() => {
    openUrlExternal('https://deus.so/download');
  }, []);
  ...
  const autoLaunchAppUrl = useMemo(() => {
    ...
    if (path === WalletConnectUniversalLinkPath && queryParams?.uri) {
      const linkUrl = walletConnectUtils.buildDeusWalletConnectDeepLinkUrl({
    ...
    <Center w="full" h="full" bg="background-default">
      <Center maxW="375px" w="full" p="6">
        ...
        <Button
          ...
          onPress={onLaunchApp}
        >
          {intl.formatMessage({ id: 'title__launch_deus' })}
        </Button>
        <Typography.Body1Strong mt="16">
          {intl.formatMessage({ id: 'title__havent_installed_deus' })}
        </Typography.Body1Strong>
        <Button
          size="xl"
          w="full"
