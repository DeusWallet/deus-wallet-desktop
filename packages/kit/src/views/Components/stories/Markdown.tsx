import { Box, Center, Markdown, ScrollView } from '@deushq/components';

const MarkdownGallery = () => {
  const md = `
  # Deus App Monorepo [English]
  ...
  When you need to install a dependency for a subdirectory, just use \`yarn workspace @deushq/web add axios\`. With a prefix like \`yarn workspace @deushq/web\`, the axios module can eventually be installed in the root directory in the web subproject.
  ...
  - \`yarn web\`: 开发网页模式，会启动静态服务器在本地 3000 端口，需同时本地启动[connect](https://github.com/DeusHQ/connect) 项目
  ...
  当需要给某一个子目录安装依赖时，只需要使用 \`yarn workspace @deushq/web add axios\`。通过 \`yarn workspace @deushq/web\` 这样的前缀，最终可以在根目录把 axios 模块安装到 web 这个子项目当中。
