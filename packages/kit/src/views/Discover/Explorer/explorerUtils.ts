...
import type { IElectronWebView } from '@deusfe/cross-inpage-provider-types';
...
(function(){
  if (window.WebSocket) {
    if (!window.$$deusWebSocketSend) {
      window.$$deusWebSocketSend = window.WebSocket.prototype.send;
    }
...
(function(){
  if (
    window.WebSocket &&
    window.$$deusWebSocketSend
  ) {
    window.WebSocket.prototype.send = window.$$deusWebSocketSend;
  }
})()
`;
