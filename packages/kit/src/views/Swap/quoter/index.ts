...
import { getNetworkImpl } from '@deushq/engine/src/managers/network';
import type { Token } from '@deushq/engine/src/types/token';
import type { IEncodedTxAptos } from '@deushq/engine/src/vaults/impl/apt/types';
import type { IEncodedTxBtc } from '@deushq/engine/src/vaults/impl/btc/types';
import type { IEncodedTxEvm } from '@deushq/engine/src/vaults/impl/evm/Vault';
import { IDecodedTxStatus } from '@deushq/engine/src/vaults/types';
import { DeusNetwork } from '@deushq/shared/src/config/networkIds';
import { IMPL_APTOS, IMPL_EVM } from '@deushq/shared/src/engine/engineConsts';
...
export class SwapQuoter {
  ...
  async convertThorswapOrderToTransaction(
    ...
    if (
      !tokenIn.tokenIdOnNetwork &&
      [DeusNetwork.cosmoshub].includes(networkIn.id)
    ) {
      result = await backgroundApiProxy.engine.buildEncodedTxFromTransfer({
        networkId: networkIn.id,
        accountId: activeAccount.id,
  ...
  parseRequestId(res: AxiosResponse<any, any>): string | undefined {
    try {
      ...
      let requestId = headers?.['x-deus-request-id'] as string | undefined;
      if (!requestId) {
        const data = headers?.['X-Request-By'];
        if (typeof data === 'string') {
          const meta = JSON.parse(data);
          // eslint-disable-next-line @typescript-eslint/no-unsafe-return, @typescript-eslint/no-unsafe-member-access
          requestId = meta['x-deus-request-id'];
        }
      }
      return requestId;
    } catch (e: any) {
