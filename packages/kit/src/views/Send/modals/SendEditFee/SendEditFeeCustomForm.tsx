...
import { getBlockNativeGasInfo } from '@deushq/engine/src/managers/blockNative';
import type { BlockNativeGasInfo } from '@deushq/engine/src/types/blockNative';
import type { EIP1559Fee } from '@deushq/engine/src/types/network';
import type {
  IEncodedTx,
  IFeeInfoPayload,
...
function printError(error: DeusError | any) {
  const e = error as DeusError;
  console.error({
    message: e.message,
    key: e.key,
    info: e.info,
            ...
            rules={{
              validate: async (value) => {
                ...
                try {
                  ...
                  printError(error);
                  const e = error as DeusValidatorError;
                  if (
                    e?.className === DeusErrorClassNames.DeusValidatorError
                  ) {
                    setMaxFeeTip({
                      type: 'error',
                  ...
                  if (
                    e?.className === DeusErrorClassNames.DeusValidatorTip
                  ) {
                    setMaxFeeTip({
                      type: 'warn',
                      message: intl.formatMessage(
              ...
              rules={{
                validate: async (value) => {
                  ...
                  try {
                    ...
                    if (
                      e?.className ===
                      DeusErrorClassNames.DeusValidatorError
                    ) {
          ...
          rules={{
            validate: async (value) => {
              ...
              try {
                ...
                if (e?.className === DeusErrorClassNames.DeusValidatorTip) {
                  setGasLimitTip({
                    type: 'warn',
                    message: intl.formatMessage(
                      {
