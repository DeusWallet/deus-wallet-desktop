...
import type { CustomAlert } from '@deushq/kit/src/views/Send/modals/SendEditFee/SendEditFeeCustomForm';
  ...
  const validateFeeRate = useCallback(
    async ({
      ...
      try {
        ...
        throw new DeusValidatorError('msg__insufficient_balance');
      }
      ...
      if (
        ...
        throw new DeusValidatorTip('msg__fee_rate_is_higher_than_necessary');
      }
    },
  ...
  const feeRateFormValidator = useCallback(
    async ({ value }: { value: string }) => {
      ...
      try {
        ...
        if (e?.className === DeusErrorClassNames.DeusValidatorError) {
          setGasPriceTip({
            type: 'error',
