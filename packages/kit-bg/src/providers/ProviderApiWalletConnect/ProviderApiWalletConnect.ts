...
import unlockUtils from '@deushq/kit/src/components/AppLock/unlockUtils';
import type { DeusWalletConnector } from '@deushq/kit/src/components/WalletConnect/DeusWalletConnector';
import type { IWalletConnectRequestOptions } from '@deushq/kit/src/components/WalletConnect/types';
import walletConnectUtils from '@deushq/kit/src/components/WalletConnect/utils/walletConnectUtils';
import type {
  IWalletConnectClientEventDestroy,
  IWalletConnectClientEventRpc,
} from '@deushq/kit/src/components/WalletConnect/WalletConnectClient';
import type { ISessionStatusPro } from '@deushq/kit/src/components/WalletConnect/WalletConnectClientForDapp';
import { WalletConnectClientForWallet } from '@deushq/kit/src/components/WalletConnect/WalletConnectClientForWallet';
import {
  backgroundShowToast,
...
class ProviderApiWalletConnect extends WalletConnectClientForWallet {
  ...
  handleSessionRequest({
    ...
    connector?: DeusWalletConnector; // v1
  ...
  handleSessionRequestV2 = async (request: Web3WalletTypes.SessionRequest) => {
    ...
    try {
      ...
      if (!walletConnectUtils.isEvmNamespaceV2(peerNamespace)) {
        throw new Error(
          `Deus WalletConnect peer namespace support EVM chain only`,
        );
      }
      if (!peerChainId) {
        throw new Error(
      ...
      if (String(currentChainId) !== peerChainId) {
        ...
        throw new Error(
          `Calling [${params?.request?.method}] ERROR:  Mismatched networks. Please switch Deus Wallet current network to chainId=${peerChainId}`,
        );
      }
