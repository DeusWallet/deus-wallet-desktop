...
import { timeout } from '@deushq/kit/src/utils/helper';
import { scanFromURLAsync } from '@deushq/kit/src/views/ScanQrcode/scanFromURLAsync';
import {
  backgroundClass,
  providerApiMethod,
...
} from '@deushq/shared/src/logger/debugLogger';
...
import type {
  IWebViewWrapperRef,
  JsBridgeDesktopHost,
} from '@deusfe/deus-cross-webview';

export interface IDeusWalletInfo {
...
class ProviderApiPrivate extends ProviderApiBase {
  ...
  */
  ...
  // $deus.$private.request({method:'wallet_sendSiteMetadata'})
  @providerApiMethod()
  ...
  getWalletInfo(): IDeusWalletInfo {
    ...
    return {
      ...
      platform: process.env.DEUS_PLATFORM,
  ...
  // $deus.$private.request({method:'wallet_getConnectWalletInfo'})
  @providerApiMethod()
  async wallet_getConnectWalletInfo(
    ...
    return {
      ...
      debugLoggerConfig: {
        ...
        /*
        window.localStorage.setItem('$$DEUS_DEBUG_LOGGER', 'jsBridge,ethereum');
