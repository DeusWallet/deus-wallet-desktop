...
import type EvmVault from '@deushq/engine/src/vaults/impl/evm/Vault';
import {
  setETHStakingApr,
  setKeleIncomes,
...
import type {
  ...
  LidoMaticOverview,
  LidoNFTStatus,
} from '@deushq/kit/src/views/Staking/typing';
import { plus, toHex } from '@deushq/kit/src/views/Swap/utils';
import {
  backgroundClass,
  backgroundMethod,
} from '@deushq/shared/src/background/backgroundDecorators';
import { DeusNetwork } from '@deushq/shared/src/config/networkIds';
import { ERC20PermitABI } from '@deushq/shared/src/contracts/abi/erc20';
import { LIDO_NFT_ABI } from '@deushq/shared/src/contracts/abi/stETH';
import {
  StMaticABI,
  poLidoNFTABI,
  stakeManagerABI,
} from '@deushq/shared/src/contracts/abi/stMatic';
...
export default class ServiceStaking extends ServiceBase {
  ...
  async buildTxForStakingETHtoKele(params: {
    ...
    return {
      data: '0xd9712d546f6e656b65790000000000000000000000000000000000000000000000000000', // bytes4(keccak256(bytes('deposit("deus")')))
  ...
  async fetchLidoOverview(params: { networkId: string; accountId: string }) {
    ...
    const { engine, serviceContract, dispatch, serviceToken } =
      this.backgroundApi;
    if (networkId !== DeusNetwork.eth && networkId !== DeusNetwork.goerli) {
      return;
    }
  ...
  private getStEthEip712Domain(networdId: string) {
    return {
      ...
      version: '2',
      chainId: networdId === DeusNetwork.eth ? 1 : 5,
  ...
  async getStEthToken(params: { networkId: string }) {
    ...
    const baseToken: Token = {
      ...
      decimals: 18,
      logoURI:
        'https://common.deus-asset.com/token/evm-1/0xae7ab96520de3a18e5e111b5eaab095312d7fe84.png',
    };
    if (networkId === DeusNetwork.eth) {
      baseToken.id = `${DeusNetwork.eth}--${MainnetLidoContractAddress}`;
      baseToken.networkId = DeusNetwork.eth;
      baseToken.tokenIdOnNetwork = MainnetLidoContractAddress;
    } else if (networkId === DeusNetwork.goerli) {
      baseToken.id = `${DeusNetwork.goerli}--${TestnetLidoContractAddress}`;
      baseToken.networkId = DeusNetwork.goerli;
