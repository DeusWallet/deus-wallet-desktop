...
import {
  ...
  setTypedPrice,
  setTypedValue,
} from '@deushq/kit/src/store/reducers/limitOrder';
import {
...
import {
  ...
  wrapToken,
} from '@deushq/kit/src/views/Swap/config';
import type {
...
class ServiceLimitOrder extends ServiceBase {
  ...
  async setDefaultTokens() {
    ...
    let limitOrderInputToken: Token = WETH9[DeusNetwork.eth];
  ...
  async syncAccount({ accountId }: { accountId: string }) {
    this.syncAccountOrders({ networkId: DeusNetwork.eth, accountId });
    this.syncAccountOrders({ networkId: DeusNetwork.bsc, accountId });
    this.syncAccountOrders({ networkId: DeusNetwork.polygon, accountId });
  }
  ...
  isStableToken(token: Token) {
    ...
    if (token.networkId === DeusNetwork.eth) {
      const tokens = [
    ...
    if (token.networkId === DeusNetwork.bsc) {
      const tokens = [
    ...
    if (token.networkId === DeusNetwork.polygon) {
      const tokens = [
