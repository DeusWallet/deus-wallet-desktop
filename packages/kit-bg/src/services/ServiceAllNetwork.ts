...
import { getPath } from '@deushq/engine/src/managers/derivation';
...
import { networkIsPreset } from '@deushq/engine/src/presets';
...
import type { NetworkWithAccounts } from '@deushq/kit/src/views/ManageNetworks/types';
...
import { FAKE_ALL_NETWORK } from '@deushq/shared/src/config/fakeNetwork';
  ...
  async generateAllNetworksWalletAccounts({
    ...
    const networks = appSelector((s) => s.runtime.networks ?? []).filter(
      (n) =>
        ...
        ![DeusNetwork.fevm, DeusNetwork.cfxespace].includes(n.id),
    );
