...
import {
  ...
  setIsAvailable,
  setNotInProgress,
} from '@deushq/kit/src/store/reducers/cloudBackup';
import { create } from '@deushq/kit/src/store/reducers/contacts';
import type { Contact } from '@deushq/kit/src/store/reducers/contacts';
import { release } from '@deushq/kit/src/store/reducers/data';
import { addBookmark } from '@deushq/kit/src/store/reducers/discover';
import { setEnableLocalAuthentication } from '@deushq/kit/src/store/reducers/settings';
import { unlock } from '@deushq/kit/src/store/reducers/status';
import {
  hasHardwareSupported,
  savePassword,
} from '@deushq/kit/src/utils/localAuthentication';
import { parseCloudData } from '@deushq/kit/src/views/Onboarding/screens/Migration/util';
import {
  backgroundClass,
  backgroundMethod,
} from '@deushq/shared/src/background/backgroundDecorators';
import * as CloudFs from '@deushq/shared/src/cloudfs';
import {
  AppEventBusNames,
  appEventBus,
} from '@deushq/shared/src/eventBus/appEventBus';
import debugLogger from '@deushq/shared/src/logger/debugLogger';
import RNFS from '@deushq/shared/src/modules3rdParty/react-native-fs';
import platformEnv from '@deushq/shared/src/platformEnv';
import { RestoreResult } from '@deushq/shared/src/services/ServiceCloudBackup/ServiceCloudBackup.enums';
import type {
  BackupedContacts,
  IBackupItemSummary,
  ISimpleDBBackUp,
  PublicBackupData,
} from '@deushq/shared/src/services/ServiceCloudBackup/ServiceCloudBackup.types';
import { memoizee } from '@deushq/shared/src/utils/cacheUtils';

import ServiceBase from './ServiceBase';

// uuid.v5('deus', '00000000-0000-0000-0000-000000000000')
// const DEUS_NAMESPACE = '30303338-6435-5664-a334-323538396638';
// uuid.v5('Contact', DEUS_NAMESPACE)
const CONTACT_NAMESPACE = '63363334-3563-5463-a336-666666353665';
// uuid.v5('Backup device', DEUS_NAMESPACE)
...
class ServiceCloudBackup extends ServiceBase {
  ...
  private getBackupPath(backupUUID: string) {
    return `deus/${this.getBackupFilename(backupUUID)}`;
  }
  ...
  private listBackups = memoizee(
    async () =>
      (await CloudFs.listFiles('deus/')).map((filename) =>
