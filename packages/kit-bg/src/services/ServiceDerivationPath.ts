...
import { getNextAccountId } from '@deushq/engine/src/managers/derivation';
...
import { INDEX_PLACEHOLDER } from '@deushq/shared/src/engine/engineConsts';
...
export default class ServiceDerivationPath extends ServiceBase {
  ...
  async createNewAccount(
    ...
    if (!accountInfo) {
      throw new DeusInternalError('can not find accountInfo.');
    }
  ...
  async batchGetHWAddress({
    ...
    const bundle = fullPaths
      ? fullPaths.map((path) => ({ path, showOnDeus: confirmOnDevice }))
      : indexes.map((index) => ({
          path: template.replace(INDEX_PLACEHOLDER, index.toString()),
          showOnDeus: confirmOnDevice,
        }));
