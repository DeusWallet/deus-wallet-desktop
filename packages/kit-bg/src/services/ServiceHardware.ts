...
import type { DevicePayload } from '@deushq/engine/src/types/device';
import {
...
} from '@deushq/kit/src/store/reducers/hardware';
import {
  setDeviceUpdates,
  setDeviceVersion,
  setVerification,
...
import {
  ...
  InitIframeLoadFail,
  InitIframeTimeout,
} from '@deushq/kit/src/utils/hardware/errors';
import { wait } from '@deushq/kit/src/utils/helper';
import type {
  BLEFirmwareInfo,
  SYSFirmwareInfo,
} from '@deushq/kit/src/utils/updates/type';
import type { FirmwareType } from '@deushq/kit/src/views/Hardware/UpdateFirmware/Updating';
import {
  backgroundClass,
  backgroundMethod,
...
} from '@deushq/shared/src/device/hardwareInstance';
import { isPassphraseWallet } from '@deushq/shared/src/engine/engineUtils';
import debugLogger from '@deushq/shared/src/logger/debugLogger';
import platformEnv from '@deushq/shared/src/platformEnv';
import { createPromiseWithTimeout } from '@deushq/shared/src/utils/promiseUtils';
import { equalsIgnoreCase } from '@deushq/shared/src/utils/stringUtils';
import type {
  EDeusDomain,
  IDeusDeviceFeatures,
} from '@deushq/shared/types';
...
class ServiceHardware extends ServiceBase {
  ...
  featuresCache: Record<string, IDeusDeviceFeatures> = {};
  ...
  async getSDKInstance() {
    ...
    return getHardwareSDKInstance({
      ...
      if (!this.registeredEvents) {
        ...
        instance.on(
          DEVICE.FEATURES,
          async (features: IDeusDeviceFeatures) => {
  ...
  */
  ...
  async connect(connectId: string) {
    if (platformEnv.isNative) {
      try {
        ...
        if (e instanceof DeusHardwareError && !reconnect) {
  ...
  @backgroundMethod()
  async getConnectId(features: IDeusDeviceFeatures) {
  ...
  private async _checkFirmwareUpdate(
    ...
    if (
      currentVersion == null ||
      payload.features?.deus_version !== currentVersion
    ) {
      actions.push(
        setDeviceVersion({
          connectId,
          version: payload.features?.deus_version ?? '0.0.0',
        }),
      );
  ...
  @backgroundMethod()
  async syncDeviceLabel(features: IDeusDeviceFeatures, walletId: string) {
