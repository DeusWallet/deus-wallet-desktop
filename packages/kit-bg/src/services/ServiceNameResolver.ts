...
export default class ServiceNameResolver extends ServiceBase {
    ...
    const NAME_RESOLVER = [
      ...
      },
      {
        ...
        supportImplsMap: {
          ...
          [DeusNetwork.trx]: ['TRX'],
          [DeusNetwork.xrp]: ['XRP'],
          [DeusNetwork.cosmoshub]: ['ATOM'],
          [DeusNetwork.fetch]: ['FET.version.FETCHAI'],
        },
  ...
  async resolveENS(name: string): Promise<ResolverNameList | null | string> {
    ...
    const chainOnlyVault = await engine.getChainOnlyVault(DeusNetwork.eth);
    try {
    ...
    try {
      for (const net of netWorks) {
        const chainOnlyVault = await engine.getChainOnlyVault(
          Object.getOwnPropertyDescriptor(DeusNetwork, net)?.value,
        );
  ...
  async resolveFilEvm(name: string) {
    ...
    const chainOnlyVault = await engine.getChainOnlyVault(DeusNetwork.fil);
