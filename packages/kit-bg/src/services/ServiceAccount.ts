...
import { HW_PASSPHRASE_WALLET_MAX_NUM } from '@deushq/engine/src/limits';
import {
  getWalletTypeFromAccountId,
  isAccountCompatibleWithNetwork,
} from '@deushq/engine/src/managers/account';
import {
  generateNetworkIdByChainId,
  getCoinTypeFromNetworkId,
  isAllNetworks,
  parseNetworkId,
} from '@deushq/engine/src/managers/network';
import { isWalletCompatibleAllNetworks } from '@deushq/engine/src/managers/wallet';
import type { IAccount, INetwork, IWallet } from '@deushq/engine/src/types';
import type {
  Account,
  DBAccount,
  DBVariantAccount,
} from '@deushq/engine/src/types/account';
import { AccountType } from '@deushq/engine/src/types/account';
import type { Network } from '@deushq/engine/src/types/network';
import type { Wallet, WalletType } from '@deushq/engine/src/types/wallet';
import { getActiveWalletAccount } from '@deushq/kit/src/hooks';
import { getManageNetworks } from '@deushq/kit/src/hooks/crossHooks';
import { passwordSet, release } from '@deushq/kit/src/store/reducers/data';
import {
  changeActiveAccount,
  changeActiveExternalWalletName,
  setActiveIds,
} from '@deushq/kit/src/store/reducers/general';
...
import {
  ...
  updateWallet,
  updateWallets,
} from '@deushq/kit/src/store/reducers/runtime';
import {
  forgetPassphraseWallet,
  rememberPassphraseWallet,
  setEnableAppLock,
  setRefreshTS,
} from '@deushq/kit/src/store/reducers/settings';
import {
  setBoardingCompleted,
  setBoardingNotCompleted,
  unlock,
} from '@deushq/kit/src/store/reducers/status';
import { DeviceNotOpenedPassphrase } from '@deushq/kit/src/utils/hardware/errors';
import { wait } from '@deushq/kit/src/utils/helper';
import { getNetworkIdImpl } from '@deushq/kit/src/views/Swap/utils';
import {
  backgroundClass,
  backgroundMethod,
  bindThis,
...
import {
  AppEventBusNames,
  appEventBus,
} from '@deushq/shared/src/eventBus/appEventBus';
import debugLogger from '@deushq/shared/src/logger/debugLogger';
import { startTrace, stopTrace } from '@deushq/shared/src/perf/perfTrace';
...
import { randomAvatar } from '@deushq/shared/src/utils/emojiUtils';
import { equalsIgnoreCase } from '@deushq/shared/src/utils/stringUtils';
...
class ServiceAccount extends ServiceBase {
  ...
  async addTemporaryWatchAccount({ address }: { address: string }) {
    ...
    const networkId = DeusNetwork.eth;
  ...
  async createHwSingleWallet({
    ...
    if (walletExistButNoAccount) {
      ...
      try {
        ...
        if (className === DeusErrorClassNames.DeusAlreadyExistWalletError) {
  ...
  async createHWWallet({
    ...
    if (passphraseState) {
      ...
      throw new DeusAlreadyExistWalletError(
