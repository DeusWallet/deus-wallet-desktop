...
import { DeusError } from '@deushq/engine/src/errors';
import { mnemonicFromEntropy } from '@deushq/engine/src/secret';
import type { Account } from '@deushq/engine/src/types/account';
import connectors from '@deushq/engine/src/vaults/impl/lightning-network/connectors';
import {
  getLnurlDetails,
  getPathSuffix,
  verifyInvoice,
} from '@deushq/engine/src/vaults/impl/lightning-network/helper/lnurl';
import HashKeySigner from '@deushq/engine/src/vaults/impl/lightning-network/helper/signer';
import type { IEncodedTxLightning } from '@deushq/engine/src/vaults/impl/lightning-network/types';
import type {
  LNURLDetails,
  LNURLError,
  LNURLPaymentInfo,
} from '@deushq/engine/src/vaults/impl/lightning-network/types/lnurl';
import type { BalanceResponse } from '@deushq/engine/src/vaults/impl/lightning-network/types/webln';
import type VaultLightning from '@deushq/engine/src/vaults/impl/lightning-network/Vault';
import type { IEncodedTx } from '@deushq/engine/src/vaults/types';
import { getBitcoinBip32 } from '@deushq/engine/src/vaults/utils/btcForkChain/utils';
import {
  backgroundClass,
  backgroundMethod,
...
export default class ServiceLightningNetwork extends ServiceBase {
  ...
  async fetchLnurlPayRequestResult({
    ...
    try {
      ...
      if (response.status >= 500) {
        throw new DeusError('Recipient server error');
      }

      if (!Object.prototype.hasOwnProperty.call(response.data, 'pr')) {
        throw new DeusError((response.data as LNURLError).reason);
      }
  ...
  async fetchLnurlWithdrawRequestResult({
    ...
    try {
      ...
      throw new DeusError(response.data.reason);
    } catch (e) {
      console.error(e);
