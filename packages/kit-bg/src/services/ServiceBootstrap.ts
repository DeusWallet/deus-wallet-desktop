...
} from '@deushq/engine/src/managers/impl';
import { getPresetNetworks } from '@deushq/engine/src/presets';
import type { DBUTXOAccount } from '@deushq/engine/src/types/account';
import { getBip32FromBase58 } from '@deushq/engine/src/vaults/utils/btcForkChain/utils';
import {
  setAccountDerivationDbMigrationVersion,
  setMigrationVersions,
} from '@deushq/kit/src/store/reducers/settings';
import { updateAutoSwitchDefaultRpcAtVersion } from '@deushq/kit/src/store/reducers/status';
import {
  backgroundClass,
  backgroundMethod,
  bindThis,
...
import { isWatchingAccount } from '@deushq/shared/src/engine/engineUtils';
import debugLogger from '@deushq/shared/src/logger/debugLogger';
import { processMigration } from '@deushq/shared/src/services/ServiceBootstrap/ServiceBootstrap.util';
import type { IServerNetwork } from '@deushq/shared/types';
...
const defaultNetworkRpcs: Record<string, string> = {
  ...
  'bch--0': 'https://fiat.deuscn.com/book/bch',
  'btc--0': 'https://rpc.deus.so/btc/',
  ...
  'ltc--0': 'https://fiat.deuscn.com/book/ltc',
  'doge--0': 'https://fiat.deuscn.com/book/doge',
...
export default class ServiceBootstrap extends ServiceBase {
  ...
  async fetchAppConfig() {
    ...
    if (networks) {
      serviceNetwork.migrateServerNetworks(networks).then(() => {
        this.switchDefaultRpcToDeusRpcNode();
      });
    }
  }
  ...
  @backgroundMethod()
  async switchDefaultRpcToDeusRpcNode() {
    ...
    for (const n of networks) {
      ...
      const deusRpc = Object.values(getPresetNetworks())
        .find((item) => item.id === n.id)
        ?.rpcURLs?.find((rpc) =>
          rpc.url?.startsWith('https://node.deus.so/'),
        )?.url;
      ...
      if (
        ...
        await serviceNetwork.updateNetwork(n.id, { rpcURL: deusRpc }, false);
      }
    }
    dispatch(
