...
import { buildModalRouteParams } from '@deushq/kit/src/hooks/useAutoNavigateOnMount';
import type {
  IDappConnectionParams,
  IDappInscribeTransferParams,
  IDappSignAndSendParams,
} from '@deushq/kit/src/hooks/useDappParams';
import {
...
} from '@deushq/kit/src/store/reducers/dapp';
import {
...
import { isSendModalRouteExisting } from '@deushq/kit/src/utils/routeUtils';
import {
  backgroundClass,
  backgroundMethod,
...
import urlUtils from '@deushq/shared/src/utils/urlUtils';
...
class ServiceDapp extends ServiceBase {
  ...
  async openModal({
    ...
    const notMatchedErrorMessage = `Deus Wallet chain/network not matched. method=${requestMethod} scope=${
      request.scope || ''
    } origin=${request.origin || ''}`;
