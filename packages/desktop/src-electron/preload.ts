...
ipcRenderer.on(
  ...
  (
    ...
    // for DesktopWebView:
    //    const { preloadJsUrl } = window.DEUS_DESKTOP_GLOBALS;
    window.DEUS_DESKTOP_GLOBALS = globals;
    // contextBridge.exposeInMainWorld('DEUS_DESKTOP_GLOBALS', globals);
  },
);

window.DEUS_DESKTOP_DEEP_LINKS = window.DEUS_DESKTOP_DEEP_LINKS || [];
ipcRenderer.on('OPEN_URL_DEEP_LINK_MESSAGE', (event, data) => {
  if (window.DEUS_DESKTOP_DEEP_LINKS) {
    window.DEUS_DESKTOP_DEEP_LINKS.push(data);
  }
  window.DEUS_DESKTOP_DEEP_LINKS = window.DEUS_DESKTOP_DEEP_LINKS.slice(-5);
});
