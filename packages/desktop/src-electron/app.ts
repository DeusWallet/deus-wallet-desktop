...
const DEUS_APP_DEEP_LINK_NAME = 'deus-wallet';
    ...
    submenu: [
      {
        label: 'Learn More',
        click: async () => {
          await shell.openExternal('https://deus.so');
        },
      },
    ],
  },
...
function createMainWindow() {
  ...
  browserWindow.webContents.on('did-finish-load', () => {
    ...
    browserWindow.webContents.send('SET_DEUS_DESKTOP_GLOBALS', {
  ...
  session.defaultSession.webRequest.onBeforeSendHeaders(
    ...
    (details, callback) => {
      ...
      if (
        ...
        // resolve deus bridge CORS error
        details.requestHeaders.Origin = 'https://jssdk.deus.so';
...
// Closing the cause context: https://deushq.atlassian.net/browse/OK-8096
...
if (process.defaultApp) {
  ...
  app.setAsDefaultProtocolClient(DEUS_APP_DEEP_LINK_NAME);
}
...
if (!app.isDefaultProtocolClient(DEUS_APP_DEEP_LINK_NAME)) {
  // Define custom protocol handler. Deep linking works on packaged versions of the application!
  app.setAsDefaultProtocolClient(DEUS_APP_DEEP_LINK_NAME);
}
