...
import type { SendConfirmPayloadInfo } from '@deushq/kit/src/views/Send/types';
...
import { checkIsUnListOrderPsbt } from '@deushq/shared/src/providerApis/ProviderApiBtc/ProviderApiBtc.utils';
import { memoizee } from '@deushq/shared/src/utils/cacheUtils';
import {
  isBRC20Token,
  parseBRC20Content,
} from '@deushq/shared/src/utils/tokenUtils';
  ...
  collectUTXOsInfo = memoizee(
    async (options: ICollectUTXOsOptions = {}) => {
      ...
      try {
        ...
        console.error(e);
        throw new DeusInternalError('Failed to get UTXOs of the account.');
      }
    },
    {
      promise: true,
  ...
  getFeeRate = memoizee(
    async () => {
      ...
      try {
        ...
        console.error(e);
        throw new DeusInternalError('Failed to get fee rates.');
      }
    },
    {
      promise: true,
