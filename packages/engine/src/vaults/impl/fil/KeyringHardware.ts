...
import { COINTYPE_FIL as COIN_TYPE } from '@deushq/shared/src/engine/engineConsts';
  ...
  async signTransaction(unsignedTx: IUnsignedTxPro): Promise<ISignedTxPro> {
    ...
    AddressSecp256k1.fromString = (address: string) => {
      ...
      if (!validateNetworkPrefix(networkPrefix))
        throw new DeusInternalError('Invalid filecoin network.');
      if (parseInt(protocolIndicator) !== ProtocolIndicator.SECP256K1)
        throw new DeusInternalError('Invalid filecoin protocol indicator.');
      ...
      if (
        ...
        throw new DeusInternalError('Invalid filecoin checksum network.');
