...
import { DeusHardwareError } from '../../../errors';
import * as DeusHardware from '../../../hardware';
...
export class KeyringHardware extends KeyringHardwareBase {
  async signTransaction(unsignedTx: UnsignedTx): Promise<SignedTx> {
    ...
    return DeusHardware.ethereumSignTransaction(
      HardwareSDK,
      connectId,
      deviceId,
      path,
  ...
  async signMessage(
    ...
    return Promise.all(
      messages.map((message) =>
        DeusHardware.ethereumSignMessage({
          HardwareSDK,
          connectId,
          deviceId,
          passphraseState,
  ...
  override async prepareAccounts(
    ...
    if (type === 'SEARCH_ACCOUNTS') {
      ...
      addressInfos = await DeusHardware.getXpubs(
        HardwareSDK,
        IMPL_EVM,
        paths,
        'address',
  ...
  async getAddress(params: IGetAddressParams): Promise<string> {
    ...
    const { connectId, deviceId } = await this.getHardwareInfo();
    const passphraseState = await this.getWalletPassphraseState();
    const address = await DeusHardware.ethereumGetAddress(
      HardwareSDK,
      connectId,
      deviceId,
      params.path,
