...
import { Geth } from '@deushq/blockchain-libs/src/provider/chains/eth/geth';
import type { Provider as EthProvider } from '@deushq/blockchain-libs/src/provider/chains/eth/provider';
...
import type {
  PartialTokenInfo,
  UnsignedTx,
} from '@deushq/engine/src/types/provider';
...
} from '@deushq/kit/src/views/Send/types';
import lib0xSequenceMulticall from '@deushq/shared/src/asyncModules/lib0xSequenceMulticall';
...
import { toBigIntHex } from '@deushq/shared/src/utils/numberUtils';
...
const EVM_L2_NETWORKS_REQUIRE_L1_FEE: string[] = [
  ...
  DeusNetwork.toptimism,
  DeusNetwork.base,
  DeusNetwork.arbitrum,
  DeusNetwork.tarbitrum,
  BASE_SEPOLIA,
  ARB_SEPOLIA,
];
...
export default class Vault extends VaultBase {
  ...
  async fetchFeeInfo(encodedTx: IEncodedTxEvm): Promise<IFeeInfo> {
    ...
    if (
      ...
      try {
        ...
        unsignedTx = await this.buildUnsignedTxFromEncodedTx({
          ...
          value: this.networkId === DeusNetwork.fevm ? '0x1' : '0x0',
        });
      } catch (e) {
        console.error(e);
      }
    ...
    if (EVM_L2_NETWORKS_REQUIRE_L1_FEE.includes(this.networkId)) {
      ...
      if (
        this.networkId === DeusNetwork.arbitrum ||
  ...
  async mmGetPublicKey(options: ISignCredentialOptions): Promise<string> {
    ...
    if (dbAccount.id.startsWith('hd-') || dbAccount.id.startsWith('imported')) {
      ...
      if (typeof password === 'undefined') {
        throw new DeusInternalError('password required');
      }
  ...
  override shouldChangeAccountWhenNetworkChanged({
    ...
    const prevNetworkIsEtc = previousNetwork?.id === DeusNetwork.etc;
    const newNetworkIsEtc = newNetwork?.id === DeusNetwork.etc;
