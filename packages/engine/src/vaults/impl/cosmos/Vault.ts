...
import type { KeyringSoftwareBase } from '@deushq/engine/src/vaults/keyring/KeyringSoftwareBase';
import {
...
import {
  addHexPrefix,
  stripHexPrefix,
} from '@deushq/engine/src/vaults/utils/hexUtils';
import { VaultBase } from '@deushq/engine/src/vaults/VaultBase';
...
import { DeusQuery } from './sdk/query/DeusQuery';
...
export default class Vault extends VaultBase {
  ...
  override async broadcastTransaction(
    ...
    try {
      ...
      if (error instanceof DeusInternalError) {
        throw error;
      }

      const { errorCode, message } = error || {};
      // eslint-disable-next-line @typescript-eslint/restrict-template-expressions
      throw new DeusInternalError(`${errorCode ?? ''} ${message}`);
    }
  }
  ...
  override async fetchOnChainHistory(options: {
    ...
    const mintScanQuery = new DeusQuery();
    const explorerTxs =
