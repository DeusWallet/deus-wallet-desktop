...
import type {
  TxInput,
  TxOutput,
} from '@deushq/engine/src/vaults/utils/btcForkChain/types';
import { convertFeeValueToGwei } from '@deushq/engine/src/vaults/utils/feeInfoUtils';
import type { IVaultInitConfig } from '@deushq/engine/src/vaults/VaultBase';
...
import { groupBy } from '@deushq/shared/src/utils/arrayUtils';
...
export default class Vault extends VaultBase {
  ...
  override async buildEncodedTxFromTransfer(
    ...
    if (tokenAddress) {
      ...
      if (!token) {
        throw new DeusInternalError('Invalid token address');
      }
  ...
  override async buildUnsignedTxFromEncodedTx(
    ...
    if (
      ...
      throw new DeusInternalError('Fee is too high');
    }
  ...
  async decodeTx(
    ...
    if (!toAddressOutput) {
      throw new DeusInternalError('No to address output found');
    }
