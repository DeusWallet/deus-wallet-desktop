...
import { decrypt } from '@deushq/engine/src/secret/encryptors/aes256';
import { TransactionStatus } from '@deushq/engine/src/types/provider';
import type { PartialTokenInfo } from '@deushq/engine/src/types/provider';
import type { Token } from '@deushq/kit/src/store/typings';
import {
  getTimeDurationMs,
  getTimeStamp,
} from '@deushq/kit/src/utils/helper';
import { log } from '@deushq/shared/src/crashlytics/index.web';
...
import { DeusJsonRpcProvider } from './provider/DeusJsonRpcProvider';
...
export default class Vault extends VaultBase {
  ...
  getSuiClient(url: string) {
    ...
    return new DeusJsonRpcProvider(
      new Connection({
        fullnode: url,
  ...
  override async attachFeeInfoToEncodedTx(params: {
    ...
    if (typeof price !== 'undefined' && typeof price !== 'string') {
      throw new DeusInternalError('Invalid gas price.');
    }
    if (typeof limit !== 'string') {
      throw new DeusInternalError('Invalid fee limit');
    }
  ...
  override async decodeTx(
    ...
    if (!transactionBlock) throw new DeusError('Invalid transaction data.');
          ...
          case 'TransferObjects': {
            ...
            if (action && typeof action.recipient === 'string') {
              ...
              if (!action.isNative) {
                ...
                if (!action.coinType)
                  throw new DeusInternalError('Invalid coin type');
                token = await this.engine.ensureTokenInDB(
                  this.networkId,
                  normalizeSuiCoinType(action.coinType),
                );

                if (!token) throw new DeusInternalError('Invalid coin type');
              }
              actions.push({
                type: action.type,
                [actionKey]: {
  ...
  override async buildEncodedTxFromTransfer(
    ...
    if (isSuiTransfer) {
      ...
      if (typeof token === 'undefined') {
        throw new DeusInternalError('Failed to get token info.');
      }
    ...
    if (!tx.blockData.gasConfig.payment) {
      ...
      do {
        ...
        if (coin) {
          ...
          throw new DeusInternalError('Insufficient balance');
        }
  ...
  async fetchFeeInfo(encodedTx: IEncodedTxSUI): Promise<IFeeInfo> {
    ...
    try {
      ...
      if (simulationTx) {
        ...
        // Only deus max send can pass, other cases must be simulated successfully
    ...
    try {
      ...
      switch (signatureScheme) {
        ...
        case 'secp256k1':
          ...
          break;
        default:
          throw new DeusInternalError('Unsupported signature scheme');
      }
      ...
      if (message.indexOf('Insufficient gas:') !== -1) {
        ...
        throw new DeusInternalError(errorMessage);
      }
    }
  }
