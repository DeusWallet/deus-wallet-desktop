...
import { COINTYPE_ADA as COIN_TYPE } from '@deushq/shared/src/engine/engineConsts';
import debugLogger from '@deushq/shared/src/logger/debugLogger';

import { DeusHardwareError, DeusInternalError } from '../../../errors';
...
import {
  transformToDeusInputs,
  transformToDeusOutputs,
...
import type { CardanoGetAddressMethodParams } from '@deusfe/hd-core';
...
export class KeyringHardware extends KeyringHardwareBase {
  ...
  override async signTransaction(unsignedTx: UnsignedTx): Promise<SignedTx> {
    ...
    if (isSignOnly && rawTxHex) {
      ...
      cardanoParams = await CardanoApi.txToDeus(
        rawTxHex,
        networkId,
        keys,
        dbAccount.xpub,
      ...
      cardanoParams = {
        ...
        outputs: transformToDeusOutputs(
          outputs,
    ...
    const res = await HardwareSDK.cardanoSignTransaction(connectId, deviceId, {
      ...passphraseState,
      inputs: transformToDeusInputs(inputs, utxos),
      derivationType,
      ...cardanoParams,
    } as any);
    if (!res.success) {
