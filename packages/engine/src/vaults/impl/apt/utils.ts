...
export async function generateUnsignedTransaction(
  ...
  if (!senderPublicKey) {
    throw new DeusHardwareError(Error('senderPublicKey is required'));
  }
  ...
  if (bscTxn) {
    ...
    if (!func) {
      throw new DeusError('generate transaction error: function is empty');
    }
...
export function convertRpcError(error: string): DeusError {
  ...
  if (
    ...
    return new DeusInternalError(error, 'msg__error_aptos_insufficient_coin');
  }
...
export function waitPendingTransaction(
  ...
  const poll: IPollFn<Promise<Types.Transaction | undefined>> = async (
    ...
    try {
      ...
      if (right) {
        ...
        if (errorCode !== 'transaction_not_found') {
          return Promise.reject(new DeusError(errorCode));
        }
      }
    }
    ...
    if (retry > retryCount) {
      return Promise.reject(new DeusError('transaction timeout'));
    }
...
export async function transactionPayloadToTxPayload(
  ...
  throw new DeusHardwareError(Error('not support'));
}
