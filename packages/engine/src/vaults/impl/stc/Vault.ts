...
import { HISTORY_CONSTS } from '@deushq/shared/src/engine/engineConsts';
...
export default class Vault extends VaultBase {
  ...
  decodedTxToLegacy(decodedTx: IDecodedTx): Promise<IDecodedTxLegacy> {
    ...
    if (type === IDecodedTxActionType.NATIVE_TRANSFER) {
      ...
      // shouldn't happen.
      throw new DeusInternalError('Incorrect decodedTx.');
    }
  ...
  async fetchFeeInfo(encodedTx: IEncodedTxSTC): Promise<IFeeInfo> {
    ...
    if (limit === '0') {
      ...
      if (!encodedTx.data) {
        ...
        throw new DeusInternalError('', 'msg__broadcast_tx_Insufficient_fee');
      }
    }
