...
import {
  IncorrectPassword,
  InvalidMnemonic,
} from '@deushq/shared/src/errors/common-errors';
  ...
  for (const testVectorBatch of bip32TestVectors) {
    ...
    for (const childIndex of testVectorBatch.path.split('/')) {
      if (childIndex === 'm') {
        ...
        let isHardenedChild = false;
        let index: number;
        if (childIndex.endsWith('h')) {
          isHardenedChild = true;
          index =
            parseInt(childIndex.slice(0, childIndex.length - 1)) + 2 ** 31;
        } else {
          index = parseInt(childIndex);
        }
        ...
        if (!isHardenedChild) {
          expect(extPub).toStrictEqual(
            CKDPub(
              ...
              {
                chainCode: Buffer.from(parent.chainCode, 'hex'),
                key: Buffer.from(parent.public, 'hex'),
              },
              index,
            ),
          );
    ...
    expect(result).toStrictEqual(testVectorBatch.keys);
  }
});
...
test('Basic signing & verifying tests', () => {
  ...
  const msgHash = Buffer.from('Hello Deus.');
...
test('Not supported curve tests', () => {
  ...
  }).toThrow(new Error(`Curve ${curveName} is not supported.`));
});
...
test('Basic mnemonic & seed tests', () => {
  for (const testVector of bip39TestVectors) {
    const rs = revealableSeedFromMnemonic(
      testVector.mnemonic,
      password,
      'TREZOR',
    );
    /* entropy is correctly generated from mnemonic. */
    const entropyWithLangPrefixed: Buffer = decrypt(
      password,
      rs.entropyWithLangPrefixed,
    );
    expect(
      entropyWithLangPrefixed
        .slice(2, 2 + entropyWithLangPrefixed[1])
        .toString('hex'),
    ).toStrictEqual(testVector.entropy);
    /* seed is correctly generated from mnemonic. */
    expect(decrypt(password, rs.seed).toString('hex')).toStrictEqual(
      testVector.seed,
    );
    /* entropy is correctly encrypted and can be used to reveal original mnemonic. */
    expect(
      mnemonicFromEntropy(rs.entropyWithLangPrefixed, password),
    ).toStrictEqual(testVector.mnemonic);
  }
});
