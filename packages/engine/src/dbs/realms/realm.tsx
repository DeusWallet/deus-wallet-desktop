...
class RealmDB implements DBAPI {
  ...
  constructor(update = false) {
    Realm.open({
      ...
      .then((realm) => {
        ...
        //   // const { PortalEntry } =
        //   //   require('@deushq/kit/src/views/Overlay/RootPortal') as typeof import('@deushq/kit/src/views/Overlay/RootPortal');
        //   // const { FULLWINDOW_OVERLAY_PORTAL } =
        //   //   require('@deushq/kit/src/utils/overlayUtils') as typeof import('@deushq/kit/src/utils/overlayUtils');
      ...
      .catch((error: any) => {
        ...
        throw new DeusInternalError('Failed to open the realm');
      });
  }
  ...
  getContext(): Promise<DeusContext | null | undefined> {
    try {
      ...
      console.error(error);
      return Promise.reject(new DeusInternalError(error));
    }
  }
  ...
  updatePassword(oldPassword: string, newPassword: string): Promise<void> {
    ...
    try {
      ...
      if (!context) {
        return Promise.reject(new DeusInternalError('Context not found.'));
      }
    ...
    try {
      ...
      if (networkFind) {
        return Promise.reject(
          new DeusInternalError(`Network ${network.id} already exist.`),
        );
      }
  ...
  updateNetwork(
    ...
    try {
      ...
      if (!network) {
        return Promise.reject(
          new DeusInternalError(`Network ${networkId} to update not found.`),
        );
      }
      this.realm!.write(() => {
        if (params.name) {
  ...
  addTokenToAccount(accountId: string, tokenId: string): Promise<Token> {
    try {
      ...
      if (!token) {
        return Promise.reject(
          new DeusInternalError(`Token ${tokenId} not found.`),
        );
      }
      this.realm!.write(() => {
      ...
      this.realm!.write(async () => {
        ...
        switch (wallet.type) {
          ...
          case WALLET_TYPE_HW: {
            ...
            if (!account.template) {
              return Promise.reject(
                new DeusInternalError(`Account should has template field`),
              );
            }
          ...
          case WALLET_TYPE_IMPORTED: {
            ...
            if (!context) {
              return Promise.reject(
                new DeusInternalError('Context not found.'),
              );
            }
  ...
  getAccountByAddress({
    ...
    try {
      ...
      if (entries.length === 0) {
        return Promise.reject(
          new DeusInternalError(`Account ${address} not found.`),
        );
      }
   ...
   * @param name
   * @returns
   * @throws { DeusInternalError, WrongPassword }
   */
  createHDWallet({
    password,
    rs,
    ...
    try {
      ...
      return Promise.reject(new DeusInternalError('Wallet creation failed.'));
    } catch (error: any) {
      console.error(error);
      return Promise.reject(new DeusInternalError(error));
    }
  ...
  async addHWWallet({
    ...
    try {
      ...
      if (!foundDevice) {
        return await Promise.reject(
          new DeusInternalError(`Device ${id} not found.`),
        );
      }
      ...
      if (error instanceof DeusHardwareError) {
        return Promise.reject(error);
      }
      return Promise.reject(new DeusInternalError(error?.message ?? ''));
    }
  }
  ...
  setWalletNameAndAvatar(
    ...
    try {
      ...
      if (
        ...
        return Promise.reject(
          new DeusInternalError('Only HD or HW wallet name can be set.'),
        );
      }
      this.realm!.write(() => {
        if (name) {
  ...
  getCredential(
    ...
    try {
      ...
      if (!credential) {
        return Promise.reject(
          new DeusInternalError(`Credential ${credentialId} not found.`),
        );
      }
  ...
  createPrivateKeyCredential(
    ...
    try {
      if (!credential) {
        return Promise.reject(new DeusInternalError('Credential required.'));
      }
      const context = this.realm!.objectForPrimaryKey<ContextSchema>(
        'Context',
        MAIN_CONTEXT,
  ...
  confirmHDWalletBackuped(walletId: string): Promise<Wallet> {
    try {
      ...
      if (wallet.type !== WALLET_TYPE_HD) {
        return Promise.reject(
          new DeusInternalError(`Wallet ${walletId} is not an HD wallet.`),
        );
      }
      if (!wallet.backuped) {
        this.realm!.write(() => {
  ...
  confirmWalletCreated(walletId: string): Promise<Wallet> {
    try {
      ...
      if (!wallet) {
        return Promise.reject(new DeusInternalError('Wallet not found.'));
      }
      const context = this.realm!.objectForPrimaryKey<ContextSchema>(
        'Context',
        MAIN_CONTEXT,
   ...
   * @param networkId
   * @param address
   * @throws {DeusInternalError, NotImplemented}
   */
  ...
  getCustomFee(networkId: string): Promise<IFeeInfoUnit> {
    try {
      ...
      if (!customFee) {
        return Promise.reject(
          new DeusInternalError(`Custom fee of ${networkId} not found.`),
        );
      }
