...
} from '@deushq/engine/src/secret';
import {
  decrypt,
  encrypt,
} from '@deushq/engine/src/secret/encryptors/aes256';
import { getShouldHideInscriptions } from '@deushq/kit/src/hooks/crossHooks/useShouldHideInscriptions';
import { appSelector } from '@deushq/kit/src/store';
import type { TokenChartData } from '@deushq/kit/src/store/reducers/tokens';
import {
  generateUUID,
  getTimeDurationMs,
} from '@deushq/kit/src/utils/helper';
import type { SendConfirmPayload } from '@deushq/kit/src/views/Send/types';
import {
  backgroundClass,
  backgroundMethod,
...
import timelinePerfTrace, {
  ETimelinePerfNames,
} from '@deushq/shared/src/perf/timelinePerfTrace';
import platformEnv from '@deushq/shared/src/platformEnv';
import { memoizee } from '@deushq/shared/src/utils/cacheUtils';
import type { Avatar } from '@deushq/shared/src/utils/emojiUtils';
import { getValidUnsignedMessage } from '@deushq/shared/src/utils/messageUtils';
...
class Engine {
  ...
  async getWallet(walletId: string): Promise<Wallet> {
    ...
    throw new DeusInternalError(`Wallet ${walletId} not found.`);
  }
  ...
  async createHDWallet({
    ...
    try {
      ...
      throw new DeusInternalError('Invalid mnemonic.');
    }
    ...
    if (
      ...
      if (isAutoAddAllNetworkAccounts) {
        ...
        networks = [autoAddAccountNetworkId || DeusNetwork.eth];
      }
  ...
  async createHWWallet({
    ...
    if (!features.initialized) {
      throw new DeusHardwareError({
        message: 'Hardware wallet not initialized.',
      });
    }
    const id = generateUUID();
    const serialNo = features.deus_serial ?? features.serial_no ?? '';
    if (id.length === 0) {
      throw new DeusInternalError('Bad device identity.');
    }
    ...
    try {
      ...
      if (e instanceof DeusHardwareError) throw e;
      throw new DeusInternalError('Failed to create HW Wallet.');
    }
  ...
  removeWallet(walletId: string, password: string): Promise<void> {
    ...
    if (!walletCanBeRemoved(walletId)) {
      throw new DeusInternalError(`Wallet ${walletId} cannot be removed.`);
    }
  ...
  async revealHDWalletMnemonic(
    ...
    if (!walletIsHD(walletId)) {
      throw new DeusInternalError(`Wallet ${walletId} is not an HD wallet.`);
    }
  ...
  async getHWAddress(accountId: string, networkId: string, walletId: string) {
    ...
    if (!device) {
      throw new DeusInternalError(`Device not found.`);
    }
    ...
    try {
      ...
      if (!address) {
        throw new DeusInternalError(`Address not found.`);
      }
      return address;
    } catch (e) {
      if (e instanceof DeusHardwareError) {
        throw e;
      } else {
        throw new DeusHardwareError({
  ...
  Promise<string> {
    ...
    const networkId = {
      ...
      '61': DeusNetwork.etc,
      '503': DeusNetwork.cfx,
      '397': DeusNetwork.near,
      '0': DeusNetwork.btc,
      '1': DeusNetwork.tbtc,
      '101010': DeusNetwork.stc,
      '501': DeusNetwork.sol,
      '195': DeusNetwork.trx,
      '637': DeusNetwork.apt,
      '3': DeusNetwork.doge,
      '2': DeusNetwork.ltc,
      '145': DeusNetwork.bch,
      '283': DeusNetwork.algo,
      '144': DeusNetwork.xrp,
      '118': DeusNetwork.cosmoshub,
      '1815': DeusNetwork.ada,
      '461': DeusNetwork.fil,
      '784': DeusNetwork.sui,
      '354': DeusNetwork.dot,
      '128': DeusNetwork.xmr,
      '111111': DeusNetwork.kaspa,
      '29223': DeusNetwork.nexa,
      '1900': DeusNetwork.xna,
      '29538': DeusNetwork.dnx,
    }[coinType];
  ...
  async addHdOrHwAccounts({
    ...
    if (!coinType) {
      throw new DeusInternalError(`coinType of impl=${impl} not found.`);
    }
  ...
  async addImportedAccount(
    ...
    if (typeof privateKey === 'undefined') {
      throw new DeusInternalError('Invalid credential to import.');
    }
  ...
  async preAddNetwork(
    ...
    if (rpcURL.length === 0) {
      throw new DeusInternalError('Empty RPC URL.');
    }
  ...
  async addNetwork(impl: string, params: AddNetworkParams): Promise<Network> {
    ...
    if (params.explorerURL) {
      try {
        ...
        console.error(error);
        throw new DeusInternalError('addNetwork invalid URL');
      }
    }
    ...
    if (typeof networkId === 'undefined') {
      throw new DeusInternalError('addNetwork: failed to get network id.');
    }
  ...
  async updateNetwork(
    ...
    if (Object.keys(params).length === 0) {
      throw new DeusInternalError('updateNetwork: params is empty.');
    }
    if (params.explorerURL) {
      try {
        const u = new URL(params.explorerURL);
        ...
        console.error(error);
        throw new DeusInternalError('updateNetwork invalid URL');
      }
    }
  ...
  async deleteNetwork(networkId: string): Promise<void> {
    if (networkIsPreset(networkId)) {
      throw new DeusInternalError('Preset network cannot be deleted.');
    }
    this.listEnabledNetworksGroupedByVault.clear();
  ...
  async restoreDataFromBackup({
    ...
    if (localPassword.length === 0 && (await this.isMasterPasswordSet())) {
      ...
      throw new DeusInternalError('Local password required.');
    }
