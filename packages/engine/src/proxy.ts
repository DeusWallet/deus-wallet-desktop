...
import { ProviderController as BaseProviderController } from '@deushq/blockchain-libs/src/provider';
import { Geth } from '@deushq/blockchain-libs/src/provider/chains/eth/geth';
import type {
  BaseClient,
  BaseProvider,
  ClientFilter,
} from '@deushq/engine/src/client/BaseClient';
import {
  N,
  sign,
  uncompressPublicKey,
...
import type { ChainInfo } from '@deushq/engine/src/types/chain';
import type { UnsignedTx } from '@deushq/engine/src/types/provider';
import type {
  Signer as ISigner,
  Verifier as IVerifier,
} from '@deushq/engine/src/types/secret';
import {
  IMPL_ALGO,
  IMPL_BCH,
  IMPL_BTC,
...
import bufferUitls from '@deushq/shared/src/utils/bufferUtils';
...
function extractResponseError(e: unknown): unknown {
  ...
  // TODO: see whether to wrap it into a gerinic DeusError.
  return e;
}
...
class ProviderController extends BaseProviderController {
  ...
  public getVerifier(networkId: string, pub: string): IVerifier {
    ...
    if (typeof provider === 'undefined') {
      throw new DeusInternalError('Provider not found.');
    }
  ...
  override async getClient(
    ...
    return Promise.reject(new DeusInternalError('Unable to init client.'));
  }
  ...
  override async getProvider(networkId: string): Promise<BaseProvider> {
    ...
    return Promise.reject(new DeusInternalError('Unable to init provider.'));
  }
