...
import type { Network } from '@deushq/kit/src/store/typings';
...
import { DeusValidatorError, DeusValidatorTip } from './errors';
...
class Validators {
  ...
  async validateGasLimit({
    ...
    try {
      ...
      if (
        ...
        throw new DeusValidatorTip('form__gas_limit_invalid_too_much');
      }
    } catch (e) {
      if (
        e instanceof DeusValidatorError ||
        e instanceof DeusValidatorTip
      ) {
        throw e;
      }
      throw new DeusValidatorError(
  ...
  async validateGasPrice({
    ...
    if (!minGasPrice && valueBN.isLessThanOrEqualTo(0)) {
      throw new DeusValidatorError('form__gas_price_invalid_min_str', {
        0: 0,
      });
    }
    ...
    if (lowValue && valueBN.isLessThan(lowValue)) {
      throw new DeusValidatorTip('form__gas_price_invalid_too_low', {});
    }
    if (
      highValue &&
      ...
      throw new DeusValidatorTip('form__gas_price_invalid_too_much', {});
    }
  ...
  async validateMaxFee({
    ...
    try {
      ...
      if (
        ...
        throw new DeusValidatorError('form__max_fee_invalid_too_low', {
          0: minAmount,
        });
      }
      const pv =
  ...
  async validateMaxPriortyFee({
    ...
    try {
      ...
      if (
        ...
        throw new DeusValidatorError('form__max_priority_fee_invalid_min', {
          0: minAmount,
        });
      }
      if (lowValue) {
