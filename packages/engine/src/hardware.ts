...
import type { IPrepareHardwareAccountsParams } from '@deushq/engine/src/vaults/types';
import { isHexString } from '@deushq/kit/src/utils/helper';
...
import * as engineUtils from '@deushq/shared/src/engine/engineUtils';
...
export async function solanaGetAddress(
  ...
  // let response;
  // try {
  //   response = await DeusConnect.solanaGetAddress(
  //     path,
  ...
  // throw new DeusHardwareError(`solanaGetAddress: ${response.payload.error}`);
...
export async function ethereumSignTransaction(
  ...
  if (
    ...
    throw new DeusInternalError('Incomplete unsigned tx.');
  }
