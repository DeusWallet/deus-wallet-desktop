...
function normalizeConfig({ platform, config }) {
  process.env.DEUS_PLATFORM = platform;
  ...
  const customAliasForComponents = (name, file) => {
    ...
    if (name.startsWith('use')) {
      return `@deushq/components/src/Provider/hooks/${name}`;
    }
    return `@deushq/components/src/${name}`;
  };
  ...
  config.plugins = [
    ...
    [
      'babel-plugin-import',
      {
        'libraryName': '@deushq/components/src',
        'camel2DashComponentName': false, // default: true
        'customName': customAliasForComponents,
      },
      '@deushq_components_src',
    ],
    [
    ...
    isDev && [
      ...
      {
        source: '@deushq/shared/src/logger/autoLogger',
        name: 'autoLogger',
        methodName: 'error',
        catchPromise: false,
        namespaced: false,
