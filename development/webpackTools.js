...
const devUtils = require('@deushq/ext/development/devUtils');
  ...
  apply(compiler) {
    compiler.hooks.done.tap(
      ...
      (compilation, callback) => {
        const msg = `Deus Build at ${new Date().toLocaleTimeString()}`;
        setTimeout(() => {
...
function normalizeConfig({
  ...
  if (platform) {
    ...
    config.plugins = [
      ...
      new webpack.DefinePlugin({
        ...
        'process.env.DEUS_BUILD_TYPE': JSON.stringify(platform),
  ...
  config.resolve.fallback = {
    ...
    'crypto': require.resolve(
      '@deushq/shared/src/modules3rdParty/cross-crypto/index.js',
    ),
